...
func main() {
	...
	for _, pos := range ninePositions {
		sum += getCheckSum(worldMap, pos, 9)
	}

	println(sum)
}

func getCheckSum(worldMap Array2D[int], position Vector, next int) int {
	...
	sum += getCheckSum(worldMap, position.Plus(Vector{X: 0, Y: 1}), next-1)
	sum += getCheckSum(worldMap, position.Plus(Vector{X: 0, Y: -1}), next-1)
	sum += getCheckSum(worldMap, position.Plus(Vector{X: 1, Y: 0}), next-1)
	sum += getCheckSum(worldMap, position.Plus(Vector{X: -1, Y: 0}), next-1)
